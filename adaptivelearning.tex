...
The reason we compute all these MISs is that Gaussian Processes in their normal setting, having as many features as we have, overfit to the data, and therefore a pre-selected small subset of features is what we feed to each Gaussian Process to prevent this overfitting. But another way of reducing the complexity of a GP is to use a covariance function such as squared exponential with automatic relevance determination (ARD) covariance function~\cite{gaussian-processes}, which we decided against after observing its running time.

Now consider the following as a module: given a feature $f_i$, compute all relevant MICs, select features according to them, and feed them to a GP. We can replace it with the following: using a nested cross-validation scheme, find the best parameters of a fast method to predict $f_i$ using other features. We use an C-SVM regression model with a linear kernel and the parameter C is set according to a nested cross-validation scheme.
